...
Displays all achievements with their lock state, progress bar, and a **Reset** button that calls `engine.reset()`, which clears all stored data so the next page load is treated as a fresh session.
